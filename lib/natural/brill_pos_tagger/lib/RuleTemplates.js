...
const ruleTemplates = {
  ...
  */
  ...
  'ADJACENT-WORDS-ARE-CD': {
    function: adjacentWordsAreCD,
    window: [-1, 1],
    nrParameter: 0
  },
...
function adjacentWordsAreCD (sentence, i) {
  if (i > 0 && i < sentence.taggedWords.length - 1) {
    if (sentence.taggedWords[i - 1].tag === 'CD') {
      let isNumber = isNumeric(sentence.taggedWords[i + 1].token)
      // Attempt to parse it as a float
      if (!isNumber) {
        isNumber = parseFloat(sentence.taggedWords[i + 1].token)
      }
      return isNumber
    }
  }
