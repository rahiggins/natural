...
const ruleTemplates = {
  ...
  */
  ...
  'CURRENT-WORD-IS-NUMBER-REALLY': {
    function: currentWordIsNumberReally,
    window: [0, 0],
    nrParameters: 0
  },
  'CURRENT-WORD-IS-URL': {
  ...
  'ADJACENT-WORDS-ARE-NUM': {
    function: adjacentWordsAreNUM,
    window: [-1, 1],
    nrParameter: 0
  },
  'VB-FOLLOWED-BY-CC-FF': {
    function: vbFollowedByCCFF,
    window: [0, 2],
    nrParameter: 1
  },
...
function currentWordIsNumberReally (sentence, i, parameter) {
  const isNumber = sentence.taggedWords[i].token.match(/^(\d+\/\d+|\d+\.\d+|\.\d+|\d+)$/) !== null
  return ((parameter === 'YES') ? isNumber : !isNumber)
}
...
function adjacentWordsAreNUM (sentence, i) {
  if (i > 0 && i < sentence.taggedWords.length - 1) {
    if (sentence.taggedWords[i - 1].tag === 'NUM') {
      return sentence.taggedWords[i + 1].token.match(/^(\d+\/\d+|\d+\.\d+|\.\d+|\d+)$/) !== null
    }
  }
  return (false)
}

function vbFollowedByCCFF (sentence, i, parameter) {
  if (i < sentence.taggedWords.length - 2) {
    if (sentence.taggedWords[i + 1].tag !== 'CC') {
      return parameter !== 'YES'
    } else {
      for (let j = i + 2; j < sentence.taggedWords.length; j++) {
        // After a conjunction, look for a food item before encountering a verb
        if (sentence.taggedWords[j].tag === 'VB') {
          return parameter !== 'YES'
        } else if (sentence.taggedWords[j].tag === 'FF') {
          return parameter === 'YES'
        }
      }
    }
