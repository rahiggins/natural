...
/*
  For the Recipe lexicon:
  For the first word in a sentence, if any of its categories is a verb tag, use the first such as the word's tag.
*/
...
class BrillPOSTagger {
  ...
  tagWithLexicon (sentence) {
    ...
    sentence.forEach(function (word, index) {
      ...
      if (that.lexicon.language === 'RE') {
        if (index === 0 && sentence.length > 1) {
          const verb = categories.find((c) => c.startsWith('VB'))
          if (verb) { category = verb }
        }
      }
      taggedSentence.addTaggedWord(word, category)
    })
